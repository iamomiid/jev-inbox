...
The provider is a setting too: `provider` is `gateway` or `typesafe` (default `gateway`), with one key per provider, `apiKey` for Vercel AI Gateway and `typesafeApiKey` for TypeSafe. The popup shows only the chosen provider's key field. The content script reads the settings by name and never the two keys: it names `provider`, `enabled`, `threshold`, `labels`, and the four view keys in its `chrome.storage.local.get` call, and reaches the cache through `getKeys()` filtered to the `crit:` prefix. A browser without `getKeys` (before Chrome 130) warms no cache and reclassifies everything through the worker.
...
- Timing: the MutationObserver callback runs the whole order pass itself, synchronously, so the reorder lands in a microtask before the next paint and Gmail's default order is never painted after a re-render. The pass is coalesced: it runs once per task, with a flag cleared in a microtask, and it only runs when a foreign record touches the visible list or an added or removed node contains a `tr.zA`. The debounced `sync()` (800 ms) only discovers rows that need classification and dispatches them; ordering never waits on it.
...
- `render()` writes nothing when the order already matches, the bar text is unchanged, and every row carries the chips signature for its current state. After its own writes it drops the observer's pending records with `observer.takeRecords()`, and the observer ignores records inside the bar or carrying only `gc-` elements, so reordering cannot retrigger itself. The observer watches `childList` only, so the animation's style writes never reach it.

## Classify flow

- The content script sends `{ type: 'classify', emails, labels }` to the service worker and receives `{ ok: true, results, errors }` or `{ ok: false, error }`. The worker settles the provider from the `provider` setting and its key from `apiKey` or `typesafeApiKey`, and results are keyed by thread id plus last message id plus provider plus the labels hash. Until the popup has stored `privacyAck: true` the worker answers `needs_ack`, the bar reads `Open Jev Inbox to finish setup.`, and classification halts until that setting arrives, so no row ever reaches a provider before the first-run notice is acknowledged. The worker also answers `{ type: 'test' }` with one boolean question on a fixed dummy state, which the popup's "Test connection" button uses and which sends no email data.
