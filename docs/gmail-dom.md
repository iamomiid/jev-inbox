...
- Classifications: a content-script map keyed by the cache key holds every classification the tab has seen. Bootstrap fills it from one `chrome.storage.local.get(null)` read, taking the `crit:` entries whose key carries the current labels hash; dispatch responses and `chrome.storage.onChanged` for `crit:` entries written by other tabs keep it current. The map survives list re-renders, scrolling, and navigation, and is cleared when the labels change or the cache is cleared, which sends those rows back through classification.
...
- Animation: a row that becomes critical and moves after a classification arrives animates with a FLIP, its old position written as a `transform` and then transitioned back over 180 ms, cleaned up after 250 ms. Moves from a Gmail re-render or any other order pass are instant, and `prefers-reduced-motion: reduce` turns the animation off.
...
- Cache: `crit:` prefixed entries in `chrome.storage.local`. Editing labels changes the hash, so the visible rows reclassify; old entries stay until "Clear cache" removes them. Only the service worker writes them; the content script reads them at bootstrap and follows later changes.
