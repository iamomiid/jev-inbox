...
- `src/popup.ts`: popup logic. Saves the provider, each provider's key, enabled, sensitivity, the first-run acknowledgement, the Show on view toggles, and the label list; runs the connection test; clears the classification cache.
